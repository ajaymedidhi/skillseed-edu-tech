...
export const api = {
  ...
  getMission: async (id: string, device_id: string) => {
    if (id === 'first') {
      const profile = await api.getProfile(device_id).catch(() => null);
      const interest = profile?.interests?.[0] || 'exploring';
      const strength = profile?.strengths?.[0] || 'creativity';
      return {
        id: 'first',
        title: `Your ${strength} spark`,
        emoji: '🔥',
        difficulty: 'Easy',
        duration_min: 5,
        growth_points: 50,
        description: `Since you love ${interest} and have a talent for ${strength}, let's start by doing a quick 5-minute creative reflection on how you can apply these today.`,
        completed: false,
        reflection_prompt: 'How did this short exercise make you feel?',
      } as Mission;
    }
    return request<Mission>(`/missions/${id}?device_id=${encodeURIComponent(device_id)}`);
  },
  completeMission: async (id: string, device_id: string) => {
    if (id === 'first') {
      const profile = await api.getProfile(device_id).catch(() => null);
      const mission = await api.getMission('first', device_id);
      return {
        mission,
        already_completed: false,
        points_gained: 50,
        trait_deltas: { curiosity: 10, creativity: 15 },
        new_milestones: [],
        profile: profile as Profile,
      } as MissionCompletion;
    }
    return request<MissionCompletion>(
