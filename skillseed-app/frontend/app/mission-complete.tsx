...
export default function MissionComplete() {
  ...
  const { patch, profile } = useProfile();
  const router = useRouter();
  const params = useLocalSearchParams<{
    title?: string; emoji?: string; traits?: string; milestones?: string; reflection?: string; missionId?: string;
  }>();

  const isFirstMission = !profile?.onboarded;
  ...
  return (
    <SafeAreaView style={[styles.wrap, { backgroundColor: colors.background }]} edges={['top', 'bottom']} testID="mission-complete-screen">
      ...
      <View style={{ padding: spacing.lg, gap: 10 }}>
        {isFirstMission ? (
          <Pressable
            testID="mission-complete-unlock"
            onPress={async () => {
              setUnlocking(true);
          ...
          >
            <Text style={styles.primaryBtnText}>{unlocking ? 'Unlocking...' : 'Unlock SkillSeed'}</Text>
          </Pressable>
        ) : (
          <>
            <Pressable
